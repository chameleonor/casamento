...
import RSVPConvidado from "@/app/confirmar/[id]/RSVPConvidado";
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return {
    ...
    description: `Confirme sua presença no casamento de Vanessa e Guilherme, dia 08 de Agosto de 2026.`,
  };
}
...
export default async function LandingConvidadoPage({ params }: Props) {
  const { id } = await params;
  const convidado = getConvidadoById(id);
